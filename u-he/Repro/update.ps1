...
function global:au_GetLatest {
  $download_page = Invoke-WebRequest -UseBasicParsing -Uri 'https://u-he.com/products/repro/'
  $regex = "$zipSuffix" + '$'
  $url = $download_page.links | ? href -match $regex | select -First 1 -expand href
  $version = ((($url.Split('/') | select -Last 1).Replace("Repro_", "")).Replace("_${zipSuffix}", "")).Split('_') | select -First 1
  ...
  $revision = ((($url.Split('/') | select -Last 1).Replace("Repro_","")).Replace("_${zipSuffix}","")).Split('_') | select -Last 1

  return @{
    Version     = $version
    Revision    = $revision
    URL         = "https://u-he.com/downloads/releases/Repro_${versionWithoutDots}_${revision}_Win.zip"
    Checksum    = $checksum
    PackageName = "uhe-repro"
  }
}
