...
function global:au_GetLatest {
  ...
  $revision = ((($url.Split('/') | select -Last 1).Replace("zebra3_","")).Replace("_${zipSuffix}","")).Split('_') | select -Last 1

  return @{
    Version     = $version
    Revision    = $revision
    URL         = "https://u-he.com/downloads/releases/zebra3_${versionWithoutDots}_${revision}_Win.zip"
    Checksum    = $checksum
    PackageName = "uhe-zebra3"
  }
}
