...
function global:au_GetLatest {
  $download_page = Invoke-WebRequest -UseBasicParsing -Uri 'https://u-he.com/products/diva/'
  $regex = "$zipSuffix" + '$'
  $url = $download_page.links | ? href -match $regex | select -First 1 -expand href
  $version = ((($url.Split('/') | select -Last 1).Replace("Diva_", "")).Replace("_${zipSuffix}", "")).Split('_') | select -First 1
  ...
  $revision = ((($url.Split('/') | select -Last 1).Replace("Diva_","")).Replace("_${zipSuffix}","")).Split('_') | select -Last 1

  return @{
    Version     = $version
    Revision    = $revision
    URL         = "https://u-he.com/downloads/releases/Diva_${versionWithoutDots}_${revision}_Win.zip"
    Checksum    = $checksum
    PackageName = "uhe-diva"
  }
}
